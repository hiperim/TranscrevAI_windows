...
from src.performance_optimizer import MultiProcessingTranscrevAI, SessionConfig
# INT8ModelConverter removed with ONNX cleanup - using simplified models
...
def get_transcription_service():
    """Lazy import transcription service"""
    try:
        from src.transcription import TranscriptionService
        return TranscriptionService
    except ImportError:
        logger.warning("TranscriptionService not available, using fallback")
        return None
...
class CompleteAppState:
    ...
    def __init__(self):
        ...
        # INT8ModelConverter removed with ONNX - using simplified model manager
    ...
    def update_user_choices(self, session_id: str, audio_input_type: Optional[str] = None):
        """Update user choices for session - PT-BR only"""
        if session_id in self.sessions:
            session = self.sessions[session_id]
            if audio_input_type:
                session["user_choices"]["audio_input_type"] = audio_input_type
    ...
    async def ensure_managers_loaded(self):
        ...
        if self.multiprocessing_enabled:
            ...
            logger.info("✅ Simple model manager ready (ONNX-free architecture)")
    ...
    async def send_message(self, session_id: str, message: Dict):
        """Send message with error handling and progress tracking"""
...
async def upload_audio_file(
    ...
    try:
        ...
        logger.info(f"Arquivo enviado: {audio_path} ({len(content)} bytes)")
...
async def handle_websocket_message(session_id: str, data: Dict):
    ...
    try:
        ...
        # User choice management - PT-BR only
            ...
            app_state.update_user_choices(session_id, audio_input_type)
            ...
            await app_state.send_message(session_id, {
                ...
                "message": f"✅ Configurações atualizadas: português brasileiro, input {audio_input_type}"
            })
