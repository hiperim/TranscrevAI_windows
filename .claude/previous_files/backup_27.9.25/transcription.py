...
class OptimizedTranscriber:
    ...
    def warm_start(self) -> float:
        ...
        try:
            # Simulate model loading (in real implementation, would use faster-whisper)
            logger.info("Loading model for warm start...")

            # Mock faster-whisper model loading
    ...
    def _get_cached_model(self, model_name: str):
        """
        Get or create cached model (Gemini cache strategy)
        """
        if model_name not in self.model_cache:
            logger.info(f"Creating cached model: {model_name}")

            # In real implementation:
            # from faster_whisper import WhisperModel
            # self.model_cache[model_name] = WhisperModel(model_name, device="cpu", compute_type="int8")

            # Mock model for testing
            self.model_cache[model_name] = f"MockModel_{model_name}_INT8"
