...
async def process_audio_pipeline(audio_path: str, session_id: str):
    ...
    try:
        ...
        if session:
                ...
                if "audio" not in session["files"]:
                    session["files"]["audio"] = audio_path

                # Store subtitles path
                session["files"]["subtitles"] = str(srt_path)

                # Note: transcript .txt is not generated by this pipeline, only SRT
                # If needed in future, generate it here and add to session["files"]["transcript"]

                logger.info(f"File paths stored in SessionManager for {session_id}: audio={session['files'].get('audio')}, subtitles={srt_path}")
