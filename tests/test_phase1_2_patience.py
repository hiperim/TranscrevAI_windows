"""
Phase 1.2: Patience Parameter Testing
Tests beam search patience to allow model to explore more transcription paths.

Expected impact: +1-3% accuracy (for complex sentences)

Tests:
    1. patience=1.0 (default - baseline)
    2. patience=2.0 (explore more paths)
    3. patience=0.5 (less exploration - faster)

Usage:
    python tests/test_phase1_2_patience.py
"""
...
async def main():
    """Run Phase 1.2 tests for patience parameter."""

    logger.info("\n" + "="*80)
    logger.info("🧪 PHASE 1.2: PATIENCE PARAMETER TESTING")
    logger.info("="*80 + "\n")

    # Test configurations
    test_configs = [
        {
            "name": "phase1_2_patience_1.0",
            "description": "Phase 1.2: patience=1.0 (default baseline)",
            "whisper_params": {
                "patience": 1.0
            }
        },
        {
            "name": "phase1_2_patience_2.0",
            "description": "Phase 1.2: patience=2.0 (explore more paths)",
            "whisper_params": {
                "patience": 2.0
            }
        },
        {
            "name": "phase1_2_patience_0.5",
            "description": "Phase 1.2: patience=0.5 (less exploration)",
            "whisper_params": {
                "patience": 0.5
            }
        }
    ...
    # Compare results
    logger.info("\n" + "="*80)
    logger.info("📊 PHASE 1.2 COMPARISON")
    logger.info("="*80 + "\n")
    ...
    for item in results:
        config_name = item['config']['name'].replace('phase1_2_', '')
    ...
    best_config = best['config']['name'].replace('phase1_2_', '')
    ...
    logger.info("\n" + "="*80)
    logger.info("✅ Phase 1.2 testing complete!")
    logger.info("="*80 + "\n")
