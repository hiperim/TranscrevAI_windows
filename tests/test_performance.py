...
def test_server_startup_time():
    """Verify server starts in <5 seconds (model caching working)"""
    import sys
    ...
    try:
        ...
        assert startup_time < 10, f"Startup took {startup_time:.2f}s (target: <10s for first run)"
...
@pytest.mark.performance
def test_model_caching_second_request_faster():
...
@pytest.mark.performance
def test_concurrent_requests_memory_stable():
